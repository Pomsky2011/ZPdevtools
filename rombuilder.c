...
int load_binary(const char* filename, uint8_t* rom, uint32_t base_addr, uint32_t max_size, int verbose) {
    ...
    bytes_read = fread(rom + base_addr, 1, size, f);
    fclose(f);
...
int main(int argc, char* argv[]) {
    ...
    rom_size = config.apu_base + get_file_size(config.apu_file);
    ...
    /* Write ROM file */
    out = fopen(config.output_file, "wb");
    if (!out) {
